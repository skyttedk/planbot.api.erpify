...
// Special development token that matches the server-side DEV_TOKEN
// Using JWT format (header.payload.signature) to pass validation checks
const DEV_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6OTk5LCJ1c2VybmFtZSI6IkRldlVzZXIiLCJlbWFpbCI6ImRldkBleGFtcGxlLmNvbSIsImlzQWRtaW4iOnRydWV9.XYZ_dev_token_bypass_auth_123456';
// Check if we're in development mode by looking at the hostname
// This works in browsers without relying on process.env which is not available client-side
...
export class SocketService {
  constructor(options = {}) {
    ...
    // Authentication
    // In development mode, use the dev token if no stored token is found
    const storedToken = this._getStoredToken();
    if (IS_DEV_MODE && !storedToken) {
      this._log("Development mode detected - using development bypass token");
      this.authToken = DEV_TOKEN;
    } else {
      this.authToken = storedToken || this.config.authToken;
    }
  ...
  _getDefaultUrl() {
    ...
    // Use the current host by default, no need for environment variables
  ...
  _getStoredToken() {
    ...
    this._log(
      `Token check - localStorage: ${localToken ? "found" : "not found"}, sessionStorage: ${sessionToken ? "found" : "not found"
      }`
    );
...
export const createSocketService = (options = {}) => {
  ...
  if (hostname === 'localhost' || hostname === '127.0.0.1' || hostname.includes('.local')) {
    ...
    // Staging environment
    url = "wss://staging-api." + hostname.split('.').slice(-2).join('.');
  } else {
    // Production environment
    url = "wss://api." + hostname.split('.').slice(-2).join('.');
  }
  ...
  return new SocketService({
    ...
    debug: hostname === 'localhost' || hostname === '127.0.0.1' || hostname.includes('.local'),
    ...options
  });
};
